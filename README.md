...
**🐳 Docker Hub:** **[tyhan55/newscortex](https://hub.docker.com/r/tyhan55/newscortex)**
...
The network is a deliberately deep **dense (fully-connected) classifier** rather than the bag-of-words count you might expect — six hidden blocks with batch normalization and dropout for regularization.
...
An **Artificial Neural Network (ANN)** is a machine learning model loosely inspired by how neurons in the brain pass signals to one another. Instead of one giant rulebook, it's built from many small, interconnected processing units that learn patterns in data.

Every neuron is connected to every neuron in the next layer — that's why it's called a **fully-connected** or **dense** network. Here's what happens inside:
...
- **Hidden layers** — each neuron takes in the values from the previous layer, multiplies them by learned **weights**, adds a **bias**, and passes the result through an **activation function** (here, ReLU).
...
**How does it "learn"?** During training, the network makes a prediction, compares it to the true label using a **loss function**, and then uses an algorithm called **backpropagation** to work backwards through the network, adjusting every weight and bias a tiny bit to reduce that loss.
...
Training accuracy climbs steadily toward ~0.98 while validation accuracy levels off around ~0.91 after the first couple of epochs. The loss chart tells the same story: training loss keeps falling, while validation loss plateaus — a classic sign of overfitting, which is why we use dropout and batch normalization.

### Classification Report

![Classification report heatmap](Screenshots/Classification_Report.png)

The model reaches an overall **91% accuracy**, with precision, recall, and F1-score fairly balanced across classes. **Sports** is the easiest desk to call (F1 ≈ 0.96) — sports headlines tend to use a distinct vocabulary. **Business** and **Sci/Tech** are trickier and occasionally confused with each other.

### Confusion Matrix

![Confusion matrix](Screenshots/Confusion_Matrix.png)

The diagonal dominates, confirming the model gets most headlines right. The most notable confusion sits between **Business** and **Sci/Tech**: 562 true Business headlines were predicted as Sci/Tech, and 445 the other way around. That makes sense — both categories talk about money, innovation, and the future.
...
The easiest way to run NewsCortex with zero setup — Docker handles all dependencies automatically.

#### Pull & Run from Docker Hub

```bash
# Pull the image
docker pull tyhan55/newscortex

# Run the container
docker run -p 8501:8501 tyhan55/newscortex
```
...
git clone https://github.com/tyhan-data/newscortex.git
cd newscortex

# Build the image
docker build -t newscortex .

# Run the container
docker run -p 8501:8501 newscortex
```
...
**That's it!** No Python installation, no virtual environment, no package conflicts. Docker handles everything.
